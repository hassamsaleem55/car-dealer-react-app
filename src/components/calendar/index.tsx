...
//   maxDate.setDate(maxDate.getDate() + 13); // next 2 weeks
...
//       cells.push({ type: "current", day: d, date });
//     }
...
//       //     slotDate.setDate(now.getDate() + (dayIndex - todayIndex));
...
//       //       if (slotDateTime.toDateString() === now.toDateString()) {
...
//         .map((item: any) => {
//           const dayIndex = item.dayOfWeek;
//           const todayIndex = TODAY.getDay(); // use UK TODAY

//           // Slot date in UK time
//           const slotDate = new Date(TODAY);
//           slotDate.setDate(TODAY.getDate() + (dayIndex - todayIndex));

//           const slots = item.slots.filter((slot: any) => {
//             // Convert slot start time to UK Date object
//             const slotDateTime = new Date(
//               slotDate.toLocaleString("en-GB", { timeZone: "Europe/London" })
//             );
//             const [h, m] = slot.startTime.split(":");
//             slotDateTime.setHours(+h, +m, 0, 0);

//             // Only future slots for today
//             if (slotDateTime.toDateString() === TODAY.toDateString()) {
...
//             let classes =
//               "h-10 sm:h-[59px] flex items-center justify-center rounded-md relative text-xs sm:text-sm ";
...
//       {/* Time Slots Section */}
//       <div className="w-full lg:w-56 rounded-xl p-4 border border-gray-200">
//         <div className="mb-2">
...
//             if (!scheduleForDay || !scheduleForDay.slots?.length) {
//               return (
...
export default function CalendarOne({
  ...
  const calendarCells = useMemo(() => {
    ...
    const cells: Array<{ type: "empty" | "current" | "next-preview"; day?: number; date?: Date }> = [];

    for (let i = 0; i < firstDayIndex; i++) cells.push({ type: "empty" });
    for (let d = 1; d <= totalDays; d++) cells.push({ type: "current", day: d, date: new Date(viewYear, viewMonth, d) });
    const remaining = 42 - cells.length;
    for (let i = 1; i <= remaining; i++) cells.push({ type: "next-preview", day: i });
  ...
  const isSelectable = (date: Date) => {
    ...
    const dayIndex = date.getDay();
    const scheduleForDay: any = timeSlots?.find((item: any) => item.dayOfWeek === dayIndex);

    return date >= todayStart && date <= maxSelectable && scheduleForDay?.slots?.length > 0;
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetchApi("/api/companies/schedule", dealerAuthToken);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
      ...
      <div className="flex-1">
        <div className="flex mb-6 justify-between items-center">
          <div className="flex justify-between items-center w-full max-w-xs space-x-2">
            <button type="button" onClick={goPrevMonth} disabled={!canGoPrev}
              className={`text-gray-500 p-1 ${!canGoPrev ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:text-gray-800"}`}>
              <ChevronLeft />
            </button>
            <h3 className="text-gray-800 text-lg sm:text-xl font-medium">
              {monthNames[viewMonth]} {viewYear}
            ...
            <button type="button" onClick={goNextMonth} disabled={!canGoNext}
              className={`text-gray-500 p-1 ${!canGoNext ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:text-gray-800"}`}>
              <ChevronRight />
            </button>
          </div>
          <button type="button" onClick={goToToday} className="cursor-pointer text-xs sm:text-sm px-3 py-1 text-primary border border-gray-200 rounded-lg hover:bg-primary/50 hover:text-black transition-all duration-300 ease-in-out">
            Today
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1 mb-4 text-center">
          {dayNamesShort.map((d) => (
            <div key={d} className="text-[10px] sm:text-xs py-1 font-medium">{d.toUpperCase()}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {calendarCells.map((cell, idx) => {
            if (cell.type !== "current") return <div key={idx} className="h-10 sm:h-[59px]" />;
            ...
            let classes = "h-10 sm:h-[59px] flex items-center justify-center rounded-md relative text-xs sm:text-sm ";
            if (!selectable) classes += "bg-gray-50 cursor-not-allowed";
            else if (isSelected) classes += "bg-primary/50";
            else classes += "border border-gray-200 hover:border-primary/80 cursor-pointer";

            return (
              <div key={idx} className={classes} onClick={() => selectable && handleSelectDay(date)}>
                {cell.day}
                {isToday && <div className="absolute bottom-1 left-1/2 -translate-x-1/2 h-1 w-1 sm:h-1.5 sm:w-1.5 rounded-full bg-primary" />}
              </div>
            );
          })}
        </div>
      ...
      <div className="w-full lg:w-56 rounded-xl p-4 border border-gray-200">
        <div className="mb-2">
          <h4 className="text-lg font-medium">Time Slots For</h4>
          {selectedDate ? <p className="text-primary font-medium">{selectedDate.toDateString()}</p> : <p className="text-gray-500">Select a date</p>}
        </div>
        ...
        {loadingSlots ? (
          ...
          if (!scheduleForDay?.slots?.length) {
            return <p className="text-gray-500 text-sm">No time slots available for this day.</p>;
          }

          return (
            <div className="grid grid-cols-1 gap-2">
              {scheduleForDay.slots.map((slot: any) => {
                const formatted = `${formatTo12Hour(slot.startTime.substring(0, 5))} - ${formatTo12Hour(slot.endTime.substring(0, 5))}`;
                const isSelected = selectedTime === formatted;

                return (
                  <div key={slot.scheduleDayId} onClick={() => setSelectedTime(formatted)}
                    className={`group flex items-center justify-between rounded-lg px-3 py-2 cursor-pointer ${isSelected ? "bg-primary/50" : "border border-gray-200 hover:border-primary/80"}`}>
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full mr-3 ${isSelected ? "bg-primary" : "bg-gray-300 group-hover:bg-primary/60"}`} />
                      <span className="text-xs sm:text-sm">{formatted}</span>
                    </div>
                  </div>
                );
